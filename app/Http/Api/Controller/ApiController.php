...
class ApiController extends InitController
{
    ...
    private function check_auth()
    {
        ...
        if (!$this->check_shop_ac($_POST, $license['token'])) {
    ...
    private function data_back($info, $msg = '', $post, $result = 'success')
    {
        ...
        if ($post == 1) {
            ...
            if (class_exists('DOMDocument')) {
                ...
                $ectouch = $doc->createElement('ectouch');
                $doc->appendChild($ectouch);

                $result = $doc->createElement('result');
                $ectouch->appendChild($result);
                $result->appendChild($doc->createCDATASection($data_arr['result']));

                $msg = $doc->createElement('msg');
                $ectouch->appendChild($msg);
                $msg->appendChild($doc->createCDATASection($data_arr['msg']));

                $info = $doc->createElement('info');
                $ectouch->appendChild($info);

                $this->create_tree($doc, $info, $data_arr['info']);
                return $doc->saveXML();
            }

            return '<?xml version="1.0" encoding="UTF-8"?>' . $this->array2xml($data_arr);
        } else {
            /* json方式 */

            return json_encode($data_arr);    //把生成的返回字符串打印出来
        }
    }
