...
function get_rank_info()
{
    ...
    if (!empty(session('user_rank'))) {
        $sql = "SELECT rank_name, special_rank FROM " . $ecs->table('user_rank') . " WHERE rank_id = '" . session('user_rank') . "'";
        $row = $db->getRow($sql);
        if (empty($row)) {
            return array();
        }
        ...
        if ($row['special_rank']) {
            ...
            $user_rank = $db->getOne("SELECT rank_points FROM " . $ecs->table('users') . " WHERE user_id = '" . session('user_id') . "'");
