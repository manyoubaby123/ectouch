...
//        }
        ...
        if ($_REQUEST['act'] == 'task') {
            ...
            if (session('task.del.start') < session('task.del.total')) {
                /* 执行操作 */
                /* 查找要删除用户 */
                $arr = $GLOBALS['db']->getCol("SELECT user_name FROM " . $GLOBALS['ecs']->table('users') . " WHERE flag = 3 LIMIT " . session('task.del.start') . ',' . $result['size']);
                ...
                /* 查找要改名用户 */
                $arr = $GLOBALS['db']->getCol("SELECT user_name FROM " . $GLOBALS['ecs']->table('users') . " WHERE flag = 2 LIMIT " . session('task.del.start') . ',' . $result['size']);
                ...
                $sql = "SELECT user_name, password, email, sex, birthday, reg_time " .
                    "FROM " . $GLOBALS['ecs']->table('users') . " LIMIT " . session('task.del.start') . ',' . $result['size'];
